...
let worldPopulationGrowth =  [|1998..2013|] |> Array.map(fun year -> year.ToString(), [ for c in countries -> c.Indicators.``Population, total``.[year]] |> List.sum) |> Chart.Line                   
let pop = [ for c in countries -> c.Name, c.Indicators.``Population, total``.[2013]] |> Chart.Geo |> Chart.WithLabels(["Name"; "Population"])
let totalPopulation2013 = [ for c in countries -> c.Indicators.``Population, total``.[2013]] |> Seq.sum

[ Title "World Population"
  TopHeader("World Population", "")                  
  H2 "Introduction"
  ...
  H2 "World map of population in 2013"
  XPlotGoogleChart pop 
  H2 "World population growth" 
